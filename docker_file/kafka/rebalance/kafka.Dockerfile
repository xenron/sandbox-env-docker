...
RUN echo "source /root/.bash_profile" > /opt/kafka/start.sh &&\
	...
	echo "sed -i 's%log.cleaner.enable=false$%#log.cleaner.enable=false%g'  /opt/kafka/kafka_2.11-"$KAFKA_VERSION"/config/server.properties" >> /opt/kafka/start.sh &&\
	...
	echo "log.cleaner.enable=true" >> /opt/kafka/kafka_2.11-$KAFKA_VERSION/config/server.properties &&\
